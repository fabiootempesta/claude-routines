...
const { runClaudeCommand } = vi.hoisted(() => ({
  runClaudeCommand: vi.fn(async () => ({ exitCode: 0, signal: null }))
}));

vi.mock("../src/server/runner", () => ({
  buildClaudeCommand: (sessionId: string) => ({
    file: "claude",
    args: ["-p", "--session-id", sessionId]
  }),
  buildClaudeResumeCommand: (sessionId: string) => ({
    file: "claude",
...
describe("Scheduler message execution", () => {
  beforeEach(async () => {
    runClaudeCommand.mockClear();
    tempDir = await mkdtemp(path.join(os.tmpdir(), "claude-routines-message-"));
  ...
  it("starts a tracked resume execution with the user's follow-up message", async () => {
    ...
    expect(execution.sessionId).toBe("019e0dc0-bc6d-7c32-8520-2f1130559c89");
