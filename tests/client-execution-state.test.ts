...
describe("client execution state", () => {
  ...
  it("allows continuing stale or failed executions because the Claude session id is always known", () => {
    const stale = execution({ status: "stale" });
    const failed = execution({ status: "failed" });
    const successful = execution({ status: "success" });

    expect(isContinuableExecution(stale)).toBe(true);
    expect(isContinuableExecution(failed)).toBe(true);
    expect(isContinuableExecution(successful)).toBe(false);
  });
});
