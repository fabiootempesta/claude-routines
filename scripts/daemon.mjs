...
async function start() {
  ...
  if (existingPid && isRunning(existingPid)) {
    console.log(`Claude Routines is already running on PID ${existingPid}.`);
    return;
  }
  ...
  console.log(`Claude Routines started on PID ${child.pid}.`);
  printUrls();
...
async function stop() {
  ...
  await rm(pidPath, { force: true });
  console.log(`Claude Routines stopped on PID ${pid}.`);
}
...
async function status() {
  ...
  if (pid && isRunning(pid)) {
    console.log(`Claude Routines is running on PID ${pid}.`);
    printUrls();
    return;
  }

  console.log("Claude Routines is not running.");
}
