    ...
    return this.mutate(() => {
      ...
      for (const execution of this.database.executions) {
        ...
        const reason = execution.processId
          ...
          : "Orphaned execution: the platform restarted or lost this execution's Claude process.";
        ...
        execution.stderr = appendLogLine(execution.stderr, `[claude-routines] ${reason}`);
...
export function createStorePath(root = process.cwd()): string {
  return process.env.CLAUDE_ROUTINES_DB ?? path.join(root, "data", "db.json");
}
