          ...
          <div className="editor-head">
            ...
            <div className="meta">
              <div>
                ...
                <span
                  className={`meta-value ${
                    isSelectedTaskRunning ? "lime" : draft.enabled ? "" : "muted"
                  }`}
                >
              ...
              <div>
                ...
                <span className="meta-value">
                  {effortLabel(draft.effort ?? DEFAULT_CLAUDE_EFFORT)}
                </span>
              </div>
              <div>
          ...
          <form className="editor-body" onSubmit={(e: FormEvent) => e.preventDefault()}>
            <div className="section">
              <div className="section-head">
                ...
                <span className="hint">where claude runs</span>
              </div>
            ...
            <div className="section">
              <div className="section-head">
                <span>Model</span>
                <span className="hint">passed as <code>--model</code> to claude</span>
              </div>
              <ModelPicker
                value={draft.model}
            ...
            <div className="section">
              <div className="section-head">
                <span>Effort</span>
                <span className="hint">claude reasoning budget</span>
              </div>
              <EffortPicker
                value={draft.effort}
            ...
            <div className="section">
              <div className="section-head">
                <span>Prompt</span>
                <span className="hint">markdown · interpreted by claude code</span>
              </div>
            ...
            <div className="danger-row">
              <Zap size={14} />
              runs as <code>claude --dangerously-skip-permissions</code> in the path above
            </div>
          </form>
        </main>
...
function EffortDots({ level }: { level: ClaudeEffort }) {
  const map: Record<string, number> = { low: 1, medium: 2, high: 3, max: 4, xhigh: 4 };
  const resolved = level ?? DEFAULT_CLAUDE_EFFORT;
  const n = resolved ? map[resolved] ?? 0 : 0;
  return (
    <span className="effort-dots" title={resolved ? effortLabel(resolved) : ""}>
      {[0, 1, 2, 3].map((i) => (
...
function Ribbon({ ticks, nextLabel }: { ticks: RibbonTick[]; nextLabel: string | null }) {
  return (
    <div className="ribbon">
      ...
      {nextLabel && (
        <span className="ribbon-label" style={{ color: "var(--lime)" }}>
          · next {nextLabel}
        </span>
      )}
    </div>
        ...
        <div className="sched-summary">
          ...
          {schedule.type === "cron" && (
            <>
              ...
              <span className="phrase-pill">
                <input
                  style={{ width: "14ch", color: "var(--lime)", textAlign: "left" }}
...
function EffortPicker(props: { value: ClaudeEffort; onChange: (value: ClaudeEffort) => void }) {
  const opts: Array<{ id: Exclude<ClaudeEffort, null>; label: string; bars: number }> = [
...
function ModelPicker(props: { value: ClaudeModel; onChange: (value: ClaudeModel) => void }) {
  const opts: Array<{ id: Exclude<ClaudeModel, null>; label: string }> = claudeModelOptions;
  return (
...
function ExecutionModal(props: {
  ...
  const canSendMessage = !isRunning;
        ...
        {(execution.status === "stale" || continuable) && (
          <div className="modal-recovery">
            ...
            <div className="recovery-body">
              ...
              {continuable ? (
                <span>
                  The platform is no longer tracking this process. Continue resumes the Claude session{" "}
                  <code>{execution.sessionId}</code> in a new tracked execution.
                </span>
              ) : (
                <span>
...
function renderExecutionOutput(execution: Execution): ReactNode {
  return (
    <>
      ...
      {"\n"}
      {execution.stdout || (execution.status === "running" ? <span className="l-dim">waiting for claude output...</span> : <span className="l-dim">(empty)</span>)}
      {execution.stderr && (
        <>
          {"\n\n"}
...
function taskToDraft(task: Task): DraftTask {
  return {
    ...
    effort: task.effort ?? DEFAULT_CLAUDE_EFFORT,
...
function normalizeDraft(draft: DraftTask): CreateTaskInput {
  return {
    ...
    effort: draft.effort ?? DEFAULT_CLAUDE_EFFORT,
    model: draft.model ?? DEFAULT_CLAUDE_MODEL
  };
}

function modelLabel(model: ClaudeModel): string {
  const resolved = model ?? DEFAULT_CLAUDE_MODEL;
  return claudeModelOptions.find((option) => option.id === resolved)?.label ?? resolved;
}
...
function effortLabel(effort: Exclude<ClaudeEffort, null>): string {
...
function buildLogText(execution: Execution): string {
  ...
  const empty = execution.status === "running" ? "waiting for claude output..." : "";
