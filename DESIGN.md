...
description: Local command center for scheduled Claude Code CLI routines.
colors:
  surface: "oklch(16.5% 0.012 60)"
  surface-muted: "oklch(20.5% 0.012 60)"
  surface-raised: "oklch(25% 0.013 60)"
  ink: "oklch(91% 0.012 60)"
  muted: "oklch(67% 0.014 60)"
  line: "oklch(32% 0.014 60)"
  accent: "oklch(73% 0.16 55)"
...
# Design System: Claude Routines

## 1. Overview

**Creative North Star: "The Local Runbook Console"**

Claude Routines is a dark-only product interface for repeatable local automation. It should feel closer to a precise operations tool than a generic AI dashboard: dense, calm, and explicit about state. The system rejects marketing gloss, decorative AI styling, and over-composed hero surfaces.

The primary experience is task authoring and monitoring. Structure should make the currently selected routine, its schedule, its effort, its working directory, and its execution history easy to connect without forcing the user into separate pages.

**Key Characteristics:**
- Compact app shell with persistent task navigation.
- Dark warm-graphite surfaces with a single operational coral accent.
...
The palette is a dark operational neutral system with one restrained coral accent and clear semantic danger/warning states. The app does not offer light mode.

### Primary
- **Run Coral** (`oklch(73% 0.16 55)`): primary actions, selected state, active status, and focus rings.

### Neutral
- **Work Surface** (`oklch(16.5% 0.012 60)`): main canvas and editor background.
- **Panel Graphite** (`oklch(20.5% 0.012 60)`): sidebars, logs list, and secondary panels.
- **Raised Graphite** (`oklch(25% 0.013 60)`): inputs, editor surfaces, selected tab fills.
- **Command Ink** (`oklch(91% 0.012 60)`): primary text.
- **Muted Nickel** (`oklch(67% 0.014 60)`): metadata, labels, secondary copy.
- **Fine Divider** (`oklch(32% 0.014 60)`): borders and separators.
...
- **Pass Green** (`oklch(72% 0.13 153)`): completed-success status only.

### Named Rules
**The One Accent Rule.** Coral is reserved for action, active state, and selection. Do not use coral as background decoration. Pass-green is reserved for "this run finished successfully" — it never carries action.
...
- **Metadata** (400-700, `0.78rem-0.86rem`, `1.25`): timestamps, counts, labels, effort chips, and secondary state.
...
- **Primary:** coral fill, dark-tinted text, 36px minimum height.
...
- **Hover / Focus:** subtle tonal shift and visible focus ring using Run Coral.
...
- **Effort chip:** uppercase 12px coral pill in the sidebar metadata when an effort is set; absent when the routine uses the CLI default.
...
- **Focus:** coral border and soft coral outline.
...
### Do:
- **Do** keep the task, schedule, effort, prompt, and logs visually connected.
...
- **Do** reserve Run Coral for primary action, selection, and active status.
